...
const createBankNew = ({ account_name,account_number,bank_number,branch_number,province_number}) => async (dispatch,getState) => {

    dispatch({ type: CREATE_BANK_NEW_REQUEST } ); 
    try {
      const { userSignin : { userInfo } } = getState();
      const { data } = await Axios.post("/api/bank-account",{ account_name,account_number,bank_number,branch_number,province_number},{
        headers:{
