...
const signin = (userName, password) => async (dispatch) => {
  ...
  try {
    if (isNaN(userName)) {
      ...
      const { data } = await axios.post('/api/auth/login-by-email', {
        email,
        password,
      });
      ...
      const { data } = await axios.post('/api/auth/login-by-phone', {
        phone_number,
        password,
      });
      dispatch({ type: USER_SIGNIN_SUCCESS, payload: data });
...
const getFullInfoUser = () => async (dispatch, getState) => {
  ...
  try {
    ...
    localStorage.setItem(
      'userInfo',
      JSON.stringify({ ...userInfo, user: data })
    );
...
const register = (phone_number, email, password, confirm_password) => async (
  dispatch
) => {
  dispatch({
    type: USER_REGISTER_REQUEST,
    payload: { phone_number, email, password },
  });
  try {
...
const updateInfoUser = (full_name, gender, birthday) => async (
  dispatch,
  getState
) => {
...
const changePwd = (new_password, confirm_new_password) => async (
  dispatch,
  getState
) => {
